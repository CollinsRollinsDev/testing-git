...
const Contact = ({ isIPhone }: Incoming) => {
  ...
  const handleForm = async (e: any) => {
    ...
    console.log(formData, "as form data");
    if (!formData.email) {
      alert('I need your email to reach back to you!');
      setBtnMsg('Send To Collins Again.')
      return;
    }
    const validMail = (mail:any) => {
      return /^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()\.,;\s@\"]+\.{0,1})+([^<>()\.,;:\s@\"]{2,}|[\d\.]+))$/.test(
        mail
      );
    }
    if(validMail(formData.email) !== true){
      alert("Email not found to be correct. Please check and try again.")
      setBtnMsg('Send To Collins Again.')
      return;
    }
    if(!formData.message || !formData.subject || !formData.name){
  ...
  return (
    <>
      ...
      >
        ...
        <form
          className={isIPhone ? styles.iphoneStyle : ""}
          onSubmit={handleForm}
        >
